# FR-7: Package Builder — Format Tour Package

## Context
Read rules/base.md before starting.
Read salebot/agent.py from FR-5 — package_builder.py will be called
...
## Dependency
FR-5 and FR-6 must be complete before starting this story.
...
class TestFormatPackage:
    ...
    def test_activities_are_numbered(self):
        """FR-7: activities start with 1. 2. 3. numbering per spec"""
...
- [ ] All previously passing FR-1 through FR-6 tests still pass (no regression)
...
## When done
Print: ✅ FR-7 complete
Do not proceed to FR-8 until all acceptance criteria above are checked.